    ...
    <div className="p-5 rounded-md text-white flex flex-col justify-center hover:scale-105 transition-all duration-300 hover:z-10 ">
      ...
      <div className="flex gap-2 mt-4">
        {blog.category.map((category) => (
          <span
            key={category}
            className="text-xs text-gray-100 bg-white/20 backdrop-blur-sm px-6 py-2 rounded-full border border-white/30 hover:bg-white/30 transition-all flex items-center gap-2 cursor-pointer"
          >
            {category}
          </span>
        ))}
